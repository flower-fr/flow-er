...
export default class DetailTab extends View
{
    constructor({ controller, context, entity, view, id, data, detailTabConfig, formJwt })
    {
        super({ controller })
        ...
        this.id = id
        this.data = data
        this.config = detailTabConfig
        this.sections = { ...detailTabConfig.layout.sections }
        this.formJwt = formJwt

        for (const section of Object.values(this.sections)) {
            if (section.renderer === "renderModalList") {
                const properties = {}
                for (const propertyId of section.properties) properties[propertyId] = data[section.entity].properties[propertyId]
                section.modalList = new ModalList({ context, entity, view, id, section, rows: data[section.entity].rows, where: data.where, order: data.order, limit: data.limit, config: this.config, properties })
            }
        }
    ...
    render = () =>
    {
        ...
        const context = this.context, entity = this.entity, data = this.data, detailTabConfig = this.config, formJwt = this.formJwt
        const html = []
        ...
        for (const section of Object.values(detailTabConfig.layout.sections)) {
        ...
        for (const [postId, post] of Object.entries(detailTabConfig.layout.posts)) {
