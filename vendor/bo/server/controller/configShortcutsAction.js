...
const { renderConfigShortcuts } = require("../view/renderConfigShortcuts")

const configShortcutsAction = async ({ req }, context, db) => {
    ...
    const properties = await getProperties(db, context, row.entity, "default", propertyDefs, [])
    ...
    return renderConfigShortcuts(context, entity, view, id, properties, value, false, "formJwt à construire")
}

const postConfigShortcutsAction = async ({ req }, context, db) => {
    const entity = assert.notEmpty(req.params, "entity")
    const id = assert.notEmpty(req.params, "id")
    const view = (req.query.view) ? req.query.view : "default"
...
module.exports = {
    configShortcutsAction,
    postConfigShortcutsAction
}