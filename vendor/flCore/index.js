...
const register = async ({ context, config, logger, app }) => {
    ...
    app.post(`${config.prefix}transaction`, execute(transactionAction, context, { sql, smtp, testMail: config.smtp?.testMail, sms, logger }))
    app.post(`${config.prefix}smtp/:id`, execute(smtpAction, context, { sql, smtp, testMail: config.smtp?.testMail, logger }))
